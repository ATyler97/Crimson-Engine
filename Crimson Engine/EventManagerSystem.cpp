...
#include <SDL2/SDL.h>
bool* stateptr;
InputSystem* Inputptr;

void EventManagerSystem::Initialize(bool* LoopState, InputSystem* Inptr)
{
	std::cout << "\t\tEvent Manager System | Initialize() | 'Event Manager System Initalizing" << std::endl;
	stateptr = LoopState;
	Inputptr = Inptr;
}
void EventManagerSystem::Update() 
{
	std::cout << "\t\tEvent Manager System | Update() | 'Checking for Updates" << std::endl;
	CheckEvents();
}
void EventManagerSystem::CheckEvents() {
	SDL_Event event;
